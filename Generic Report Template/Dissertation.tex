\documentclass[pdftex,12pt,a4paper,oneside]{report}

\usepackage[a4paper,margin=25mm]{geometry}


% === Fonts ===
\usepackage[T1]{fontenc}
\usepackage{lmodern}  % Times-like, professional look
\usepackage[british]{babel}
\usepackage{microtype}

% === Line spacing ===
\usepackage{setspace}
\onehalfspacing

% === Maths, algorithms, figures ===
\usepackage{amsmath,amssymb}
\usepackage{graphicx}
\usepackage{siunitx}
\sisetup{detect-weight=true,detect-family=true}

% === Tables & lists ===
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{enumitem}
\setlist{nosep}

% === Links & cross-refs ===
\usepackage[hidelinks]{hyperref}
\usepackage[nameinlink,capitalise]{cleveref}

% Numbering depth & ToC depth
\setcounter{secnumdepth}{3}
\setcounter{tocdepth}{3}

\usepackage{amsmath}
\usepackage{algorithm}
\usepackage{algorithmic}
\usepackage{fancyhdr}
\usepackage{graphicx}
\usepackage{lipsum}
\usepackage{multirow}
\usepackage{setspace}

\usepackage{enumitem}
\setlist[itemize]{topsep=0.5em,itemsep=0.2em,leftmargin=*}
\setlist[enumerate]{topsep=0.5em,itemsep=0.2em,label=\arabic*.,leftmargin=*}
\setlist[description]{style=nextline,topsep=0.5em,itemsep=0.2em,leftmargin=2.2em,
  font=\normalfont\bfseries}



\setlength{\headheight}{15pt} % or 16pt if warning persists
\emergencystretch=2em

\fancyhf{}
\pagestyle{fancy}
\renewcommand{\headrulewidth}{0.2pt}

% Styles can be Sonny, Lenny, Glenn, Conny, Rejne, Bjarne and Bjornstrup
\usepackage[Sonny]{fncychap} 

\begin{document}

\include{Preamble/Frontpage}
\include{Preamble/Abstract}
%\include{Preamble/Dedication} % Comment out for UG dissertations
\include{Preamble/Acknowledge}
%\include{Preamble/Declarations} % Comment out for UG dissertations
%\include{Preamble/Sponsors} % Comment out for UG dissertations
\include{Preamble/Abbreviations}
%\include{Preamble/Notations} % Comment out for most dissertations 
\include{Preamble/Contents}
\include{Preamble/Lists}
\include{Preamble/ChapterMods}

\include{Chapter1/Chapter1}
\include{Chapter2/Chapter2}
\include{Chapter3/Chapter3}
\include{Chapter4/Chapter4}
\include{Chapter5/Chapter5}
\include{Chapter6/Chapter6}
\include{Chapter7/Chapter7}
\include{Chapter8/Chapter8}


\bibliographystyle{unsrt}
\bibliography{bibliography}

\include{Appendices/Appendices}

\end{document}
