...
if __name__ == "__main__":
    ...
    print(f"🔍 Master validation for phantoms in: {data_dir}")
    if quick_mode:
        print("🚀 Running in quick mode (first 100 phantoms)")
    
    # Run comprehensive validation
    results = analyze_phantom_dataset(data_dir, quick_mode=quick_mode)
    ...
    if results['status'] == "EXCELLENT":
        print("🏆 DATASET EXCELLENT - Ready for 5,000 phantom generation!")
        print("🚀 Proceed with confidence to production training")
    elif results['status'] == "USABLE_WITH_MONITORING":
        print("✅ DATASET USABLE - Monitor flagged issues during training")
        print("📊 Overall quality acceptable for ML training")
    else:
        print("⚠️  DATASET NEEDS ATTENTION - Address issues before large-scale use")
        print("🔧 Review detailed analysis and fix critical problems")
    
    print(f"\n📊 Overall Score: {results['overall_score']:.1f}/100")
    print(f"📈 Phantoms Processed: {results['phantoms_processed']}")
    
    if results['critical_issues']:
        print(f"⚠️  Critical Issues: {', '.join(results['critical_issues'])}")
    else:
        print("✅ No Critical Issues Detected")
    
    print("\n🎯 Ready for next phase of your NIR tomography research!")
